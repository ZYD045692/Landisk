...
| [8 配置](#配置-type8) | [12 服务](#服务-type12) | [13 预览](#预览-type13) |
...
由后端写入（`/api/download?inline=1` 初始请求时，见下方说明）。

**op 码表**

| op | 含义 |
|---|---|
| 1 | 成功（视频在线播放 / Markdown 渲染） |
| 2 | 失败 |
...
成功：
```json
{"op":1,"file":"/test/video.mp4","size":"24.5 MB","root":"D:\\test"}
{"op":1,"file":"/test/readme.md","size":"3.2 KB","root":"D:\\test"}
```

失败（后端）：
```json
{"op":2,"file":"/test/nonexist.mp4","error":"系统找不到指定的文件"}
{"op":2,"file":"/test/video.mp4","error":"不能下载目录","is_dir":true}
```

失败（前端拒绝，例外写入）：
```json
{"op":2,"file":"big.md","error":"文件过大，无法预览"}
```

**渲染效果**

```
2026-07-29 18:33:58 INFO [预览] video.mp4 (24.5 MB)
```

```
2026-07-29 18:33:58 WARN [预览] 预览失败
big.md
文件过大，无法预览
```

> **防刷屏**：视频播放会发起多次 Range 请求（拖动进度条等），仅**初始请求**（无 Range 头或 Range 起始为 0）写日志，seek 请求不写，避免日志被播放刷屏。
...
| `file` | string | 文件名（从路径中提取） | 1, 4, 5, 6, 13 |
| `files` | array | 文件列表 `[{name, size?}]` | 1, 2, 4, 5 |
| `count` | number | 数量 | 1, 2, 4, 5 |
| `dest` | string | 去向 `trash` / `permanent` | 4 |
| `root` | string | 共享目录完整路径 | 1, 2, 4, 5, 6, 10, 13 |
| `size` | string | 文件大小文字（如 "1.5 MB"） | 1, 2, 5, 13 |
...
2. **例外（前端写入，界面行为）**：`type=1 op=0`（上传取消）、`type=1 op=2`（不支持上传文件夹/读取文件失败，前端直接拒绝时）、`type=4 op=0`（删除取消）、`type=7 op=5`（添加/移除已取消）、`type=11 op=0`（清空日志已取消）、`type=13 op=2`（Markdown 文件过大拒绝预览，前端直接拒绝时）
3. **日志查看器渲染**：所有文本左对齐
4. **错误消息**：`error` 字段为用户可读的中文消息，由后端根据具体错误原因生成；未知错误统一归为 `"其他原因"`
5. **批量操作日志**（`type=4`）：成功写一条聚合日志，失败每条各写一条独立日志
6. **写入方**：
   - `type 1/2/4/5/6/7/8/9/11/12/13` — 后端
   - `type 7 op=5`、`type 11 op=0`、`type=1 op=0`、`type=1 op=2`（拒绝类）、`type=4 op=0`、`type=13 op=2`（拒绝类） — 前端
   - `type 10 op=3` — 后端
